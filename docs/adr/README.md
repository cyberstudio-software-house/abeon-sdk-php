...
| [0031](0031-app-instance-per-client.md) | One instance of each business application per client | Accepted | Buying an app provisions the client's own instance and database; one host per client with apps under path prefixes; login across domains by a one-time code; instance bound to `ABEON_ORG_ID`. **Amended 2026-09-24 by ADR-0035** (platform host is the default; a router picks the instance) |
...
| [0034](0034-object-storage-on-s3.md) | Object storage speaks S3, and applications hold no credentials | Accepted | Amends ADR-0021: the layout stands, the protocol becomes S3 so the `{service}/` prefix is enforced rather than agreed, and an application holds no keys at all — it asks AbeonUnified for a signed address and moves the bytes itself |
| [0035](0035-platform-host-and-instance-router.md) | The platform host is the default address, and a router picks the instance | Accepted | Amends ADR-0031 §2–3: `app.abeon.pl` serves the panel by default and an own domain becomes optional; a router at the entry validates the token, reads `org_id` and resolves the instance; one session on the platform host, the one-time code narrowed to clients with an own domain; the root becomes a launcher |
